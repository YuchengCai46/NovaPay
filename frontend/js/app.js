...
async function freeze(id) {
  ...
  if (!pin) return;
  const r = await api('POST', '/api/cards/freeze', { card_id: id, pin });
...
async function payAndUnfreeze(id) {
  ...
  window.doPayAndUnfreeze = async (cardId) => {
    ...
    const r = await api('POST', '/api/cards/unfreeze', { card_id: cardId, payment_source: src, pin });
...
async function renewCard(id) {
  ...
  window.doRenew = async (cardId) => {
    ...
    if (btn) { btn.disabled = true; btn.textContent = `${ic('refresh')} ${t('common_processing')}`; }
    const r = await api('POST', '/api/cards/renew', { card_id: cardId, payment_source: src, pin });
...
async function cancelCard(id) { if (!confirm(t('admin_cancelCardConfirm'))) return; const r = await api('POST', '/api/cards/cancel', { card_id: id }); if (r.ok) { toast(t('cards_canceledMsg')); renderCards(); } else toast(r.data?.message, 'err'); }
...
function renderAdmin() {
  ...
  const views = [['dashboard', t('admin_view_dashboard')], ['users', t('admin_view_users')], ['cards', t('admin_view_cards')], ['transactions', t('admin_view_transactions')], ['suggestions', t('admin_view_suggestions')], ['cancellations', t('admin_view_cancellations')], ['config', t('admin_view_config')], ['audit', t('admin_view_audit')]];
  c.innerHTML = `
  ...
  ({ dashboard: adminDashboard, users: adminUsers, cards: adminCards, transactions: adminTransactions, suggestions: adminSuggestions, cancellations: adminCancellations, config: adminConfig, audit: adminAudit }[state.adminView] || adminDashboard)();
...
async function adminTransactions() {
  ...
  d.innerHTML = `<div class="glass"><table><thead><tr><th>${t('admin_id')}</th><th>${t('admin_colUser')}</th><th>${t('admin_type')}</th><th>${t('admin_amount')}</th><th>${t('admin_status')}</th><th>${t('admin_suspicious')}</th><th>${t('admin_time')}</th></tr></thead><tbody>
    ${txs.map(tx => `<tr><td>${esc(tx.id)}</td><td>${esc(tx.user_id)}</td><td>${esc(tx.type)}</td><td>${money(tx.amount)}</td><td>${esc(tx.status)}</td><td>${tx.is_suspicious ? '<span class="badge red">'+t('common_yes')+'</span>' : t('common_no')}</td><td>${esc(fmtDate(tx.created_at))}</td></tr>`).join('')}
